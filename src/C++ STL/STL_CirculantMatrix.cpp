#include "STL_CirculantMatrix.hpp"
#include <sstream>
#include <chrono>
#include <cmath>
#include <stdexcept>

std::vector<STL_CirculantMatrix::ComplexType> STL_CirculantMatrix::row_to_column(const std::vector<ComplexType>& first_row) {
...
STL_CirculantMatrix::STL_CirculantMatrix(const std::vector<ComplexType> &A, const std::vector<ComplexType> &b): A(A), b(b){}

STL_CirculantMatrix::STL_CirculantMatrix(const std::string &filename) {
...
std::vector<STL_CirculantMatrix::ComplexType> STL_CirculantMatrix::solve() {
...
void STL_CirculantMatrix::solve_and_output(const std::string &filename) {
    ...
    f_out << "Time consumed solving the system with std::complex: " << duration.count() << " seconds" << std::endl;
    ...
    for (int i = 0; i < solutions.size(); ++i) {
        double real_term = A[i].real() * solutions[i].real() - (A[i].imag() * solutions[i].imag());
        double imag_term = A[i].real() * solutions[i].imag() + A[i].imag() * solutions[i].real();
        real_result += real_term;
        imag_result += imag_term;
    }

    if (fabs(real_result - b[0].real()) < 1e-10) {
        f_out << "TRUE" << std::endl;
    } else {
        f_out << "FALSE" << std::endl;
    }
    f_out << "Expected result: " << b[0].real() << std::endl;
...
void STL_CirculantMatrix::fast_fourier_transform(std::vector<ComplexType> &vec) { //NOLINT
...
void STL_CirculantMatrix::inverse_fft(std::vector<ComplexType>& vec) {
    unsigned long n = vec.size();
    ...
    for (int i = 0; i < n; ++i) {
        vec[i] = std::conj(vec[i]) / static_cast<double>(n);
    }
